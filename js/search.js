...
// Events Listeners from the form elements
const make = document.querySelector('#make');
make.addEventListener('input', e => {
    searchFields.make = e.target.value;
    // execute the filter cars based on the itens thet we have in the search fields object
    filterCars();
});
...
function showCars(cars) {
    ...
    // Build the HTML end print the data in the conteiner
    cars.forEach(car => {
        const carHTML = document.createElement('p');
        carHTML.innerHTML = `
            ${car.make} ${car.carmodel} - ${car.year} - ${car.doors} Dors - Transmission: ${car.transmission} - Price: ${car.price} - Color: ${car.color}
        `;
